...
int min_relocation(IntDequeue *q, direction dir)
{
	...
	case both:
		...
		{
			if (q_temp->min_idx[0] == q_temp->front)
				RemoveFront(q_temp);
			else if (q_temp->min_idx[q_temp->num_min - 1] == ((q_temp->rear - 1 < 0) ? q_temp->max - 1 : q_temp->rear - 1))
				RemoveRear(q_temp);
			if (IsEmpty(q_temp))
				break;
		}
...
int pricmp(const IntDequeue *n1, const IntDequeue *n2)
{
	...
	{
		return n1->que[n1->min_idx[0]] < n2->que[n2->min_idx[0]] ? -1 : n1->que[n1->min_idx[0]] > n2->que[n2->min_idx[0]] ? 1
																														  : 0;
	}
}
